...
import { 
  CreditCard, Lock, ChevronLeft, Check, Calendar, MapPin, Users, 
  PartyPopper, Heart, Briefcase, Sun, Truck, Building, Smartphone
} from 'lucide-react'

const paymentMethods = [
  { id: 'card', label: 'Credit/Debit Card', icon: <CreditCard size={24} /> },
  { id: 'apple', label: 'Apple Pay', icon: <Smartphone size={24} /> },
...
const CheckOut = () => {
  ...
  const currentPackage = packageData[bookingData.package] || packageData.standard
  const guestFee = bookingData.guestCount > 50 ? (bookingData.guestCount - 50) * 10 : 0
  ...
  const handleSubmit = async (e) => {
    ...
    // Generate confirmation number using a simple approach
    const generateConfirmationNumber = () => {
      const timestamp = new Date().getTime()
      return `EVT-${timestamp.toString(36).toUpperCase()}`
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <h1 className="text-3xl font-bold text-slate-800 mb-8 text-center">Complete Your Booking</h1>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-200">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-semibold text-slate-800">{bookingData.eventType}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500">Package</p>
                  <p className="font-semibold text-slate-800">{currentPackage.name}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500">Date</p>
                  <p className="font-semibold text-slate-800">
                    {bookingData.eventDate ? new Date(bookingData.eventDate).toLocaleDateString() : 'Not selected'}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-slate-500">Time</p>
                  <p className="font-semibold text-slate-800">{bookingData.eventTime || 'Not selected'}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-sm text-slate-500">Venue</p>
                  <p className="font-semibold text-slate-800">{bookingData.venue}</p>
                </div>
                <div>
                  <p className="text-sm text-slate-500">Guests</p>
                  <p className="font-semibold text-slate-800">{bookingData.guestCount} guests</p>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-lg border-2 border-slate-200 focus:border-pink-500 focus:outline-n one"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-200">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {paymentMethods.map((method) => (
                  ...
                  >
                    <div className={`${paymentMethod === method.id ? 'text-pink-500' : 'text-slate-600'}`}>
                      {method.icon}
                    </div>
              ...
              {paymentMethod === 'apple' && (
                <div className="bg-slate-50 rounded-xl p-6 text-center">
                  ...
                  <p className="text-slate-600">You will be redirected to Apple Pay to complete your payment.</p>
                </div>
              )}
              ...
              {paymentMethod === 'cash' && (
                <div className="bg-slate-50 rounded-xl p-6">
                  ...
                  <p className="text-slate-600">Pay with cash upon arrival at the venue. A 50% advance deposit is required to confirm your booking.</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg border border-slate-200">
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-slate-600">
                  I agree to the <a href="#" className="text-pink-500 hover:underline">Terms and Conditions</a> and 
                  <a href="#" className="text-pink-500 hover:underline"> Privacy Policy</a>. I understand that my booking 
                  is subject to availability and cancellation policy.
                </span>
              </label>
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl p-6 shadow-xl border border-slate-200 sticky top-24">
              ...
              <div className="flex gap-4 mb-6 pb-6 border-b border-slate-200">
                ...
                <div>
                  <p className="font-semibold text-slate-800">{bookingData.eventType}</p>
                  <p className="text-sm text-slate-500">{bookingData.venue}</p>
                  <p className="text-sm text-slate-500">
                    {bookingData.eventDate ? new Date(bookingData.eventDate).toLocaleDateString() : 'TBD'}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3 mb-6">
                ...
                {guestFee > 0 && (
                  <div className="flex justify-between text-slate-600">
                    <span>Extra Guests ({bookingData.guestCount - 50} × $10)</span>
